 ...
 * Define the initial state $\Lambda$ as $|00⟩$ and calculate the final state $|\psi_f⟩$ after applying $U$ and $𝑈^{-1}$.
 
 * Use the `expected_payoff` argument from `qvirus` to calculate the expected payoffs $\pi_v$ and $\pi_V$ based on the final state and the possible outcomes from Table 2.
...
For example, $I \otimes 2$ combines the identity matrix I with a $2 \times 2$ identity matrix, effectively creating a larger 4x4 identity matrix that acts on two qubits simultaneously.
