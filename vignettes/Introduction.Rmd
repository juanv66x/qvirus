...
The previous results follow from the property that ∣$e^{i\phi}$∣ = 1 for any real value of ϕ and from the fact that the absolute value of a complex number z = a + bi is given by ∣z∣= $\sqrt{a^2 + b^2}$. Then, z = $e^{i\phi_1} = \cos{\phi_1} + i\sin{\phi_1}$. Computing the absolute gives $|e^{i\phi_1}∣= |\sqrt{\cos^2{\phi_1} + \sin^2{\phi_1}}| = \sqrt{1} = 1$.
...
To visualize the quantum states represented by the complex vectors on the Bloch sphere, for a given state $|ψ_i⟩$, the coordinates of the state are converted to spherical coordinates with given $\theta_i$ and $\phi_i$. Then a point on the Bloch sphere corresponding to these coordinates is plotted.
...
Considering the quantum state ∣ψ⟩ of the illustration example in Equation \@ref(eq:spsi) with complex amplitudes $\alpha = \frac{1+2i}{3}$ and $\beta = -\frac{2}{3}$. The probability of measuring classical state is calculated as $Prob(0) = ∣\alpha∣^2 = ∣\frac{1+2i}{3}∣^2 = \frac{5}{9}$ and classical state 1 is obtained as $Prob(1) = ∣\beta∣^2 = ∣−\frac{2}{3}∣^2 = \frac{4}{9}$, therefore, these probabilities satisfy the condition that the sum of the squared absolute values of the components in the quantum state vector equals 1.
